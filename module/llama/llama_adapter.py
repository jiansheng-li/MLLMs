...
class LLaMA_adapter(nn.Module):
    ...
    def get_trainable_params(self, phase='finetune'):
        ...
        if phase == 'finetune':
            ...
            train_param_name = ['gate', 'clip_proj', 'clip_proj_norm', 'visual_query', 'visual_blocks', 'visual_proj',
...
def load(name, llama_dir, llama_type="7B", device="cuda" if torch.cuda.is_available() else "cpu", download_root='llama/ckpts',
         max_seq_len=512,
         phase="finetune"):
    if name in _MODELS:
    ...
    logging.info(f'Loading LLaMA-Adapter from {model_path}')
