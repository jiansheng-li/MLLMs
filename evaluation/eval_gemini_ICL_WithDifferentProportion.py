...
from evaluation.utils import analyse_unified_output
...
def main(args, dataset, first_num):
    for idx, each_dataset in enumerate(args.dataset):
        ...
        for line in tqdm(questions):
            ...
            for i in range(ice_num):
                j = int(i / 2) if len(image_second) >= (ice_num - first_num) or (
                        i % 2 == 0 and len(image_first) < first_num) else int(len(ice_data_path) - 1 - i // 2)
                image_path = ice_data_path[j]
                if j <= 3:
                    image_first.append(image_path)
                else:
                    image_second.append(image_path)
                res_qs += str(image_path)
...
if __name__ == '__main__':
    ...
    for j in ['NIH-Chest', 'XCOVFour', 'CT-Xcov']:
        ...
        args.ice_num = 8
        args.output_dir = f"./exp_output/{args.model_name}/{args.dataset[0]}/first_num_{first_num}"
        if not os.path.exists(args.output_dir):
            os.makedirs(args.output_dir)
            main(args, dataset, first_num)
